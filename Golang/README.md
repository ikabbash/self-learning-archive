...
- Array is a fixed-size collection of elements of the same type, with its length being part of its type (e.g., `[3]int`), making it inflexible but efficient for fixed data. Array is also contiguous in memory.
    ...
    -  Array size is encoded in its type. For example, say you have a function that takes `[5]int`, if you try to pass an `[4]int` the function won't compile. It's basically the same trying to pass a `string` into a function that wants `int`.
        - slices do not encode the size of the collection and instead can have any size.
